...
Every Rigbox workspace is a full Linux VM with SSH access. Connect from any terminal to get a shell, transfer files, or use the interactive TUI dashboard.
...
```bash
ssh {workspace-name}-{workspace-id}@eu-west-1.rigbox.dev
```

For example, if your workspace is named `my-project` with ID `ws-ctbxi8lb`:
...
<Tip>
  The SSH command is shown in the dashboard when you create a workspace. You can also construct it from the workspace name and ID (strip the `ws-` prefix from the ID).
</Tip>
...
Rigbox includes an interactive terminal dashboard accessible over SSH. Connect as the `rig` user:

```bash
ssh rig@eu-west-1.rigbox.dev
```
...
3. You're using the correct username format: `{workspace-name}-{id}@eu-west-1.rigbox.dev`
...
3. Verify you are using the correct identity file: `ssh -v my-project-ctbxi8lb@eu-west-1.rigbox.dev`
