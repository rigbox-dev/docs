...
For zero-downtime rollouts, stage a sibling app on a staging subdomain, verify it, then atomically swap production over.
...
This deploys a sibling on a staging subdomain (suffixed with `v2`) without touching production.

**Step 2 — verify the staged app**, then promote:

```bash
rig deploy --bluegreen v2 --promote
```

`--promote` atomically swaps production to the staged sibling with zero downtime, keeping the old version around as a rollback target.

<Tip>
Blue-green pairs naturally with reproducible deploys: the staged sibling is booted from its own frozen image, so what you verify on the staging subdomain is byte-for-byte what production becomes on promote.
</Tip>
