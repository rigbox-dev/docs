...
The fastest way to create a workspace is `rig spawn` with a template ID:

```bash
rig spawn my-project --template dev
```

`rig spawn` creates the workspace, boots it, and waits until it's ready — all in one call. See [Quick Deploy](/api-reference/templates/quick-deploy) for the API form.
...
```bash
rig new heavy-compute --image dev --ram 2048 --vcpu 2 --disk 8192
```
