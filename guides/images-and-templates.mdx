...
response = requests.get(
    "https://api.rigbox.dev/api/images",
...
See [List Images](/api-reference/images/list) for the full response schema.

## Templates

Templates are pre-configured workspace definitions that pair an image with sensible resource defaults. Use them with [quick deploy](/api-reference/workspaces/quick-deploy) for one-call workspace creation.
...
```python Python
response = requests.get(
    "https://api.rigbox.dev/api/templates",
    headers={"Authorization": f"Bearer {api_key}"},
)
...
See [Quick Deploy](/api-reference/workspaces/quick-deploy) for the full request/response schema.
...
See [Resize Workspace](/api-reference/workspaces/resize) for details.
