...
When `rig deploy` takes the [reproducible path](/guides/deploying#reproducible-deploy), it freezes the `install:` result into a content-addressed image and records a release pointing at that digest. Standard (rsync-only) deploys do not produce releases — there is no frozen image to pin.
