...
const response = await fetch("https://api.rigbox.dev/api/v1/quick-deploy", {
  method: "POST",
  headers: {
...
```bash cURL
curl -X POST https://api.rigbox.dev/api/v1/apps \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```javascript JavaScript
const app = await fetch("https://api.rigbox.dev/api/v1/apps", {
  method: "POST",
  headers: {
    "Authorization": "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
...
```python Python
response = requests.post(
    "https://api.rigbox.dev/api/v1/apps",
    headers={"Authorization": "Bearer YOUR_API_KEY"},
    json={
        "workspace_id": "ws_abc123",
        "name": "user-api",
...
curl https://api.rigbox.dev/api/v1/workspaces/{workspace_id}/metrics \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
# AI usage history
curl https://api.rigbox.dev/api/v1/users/me/ai-usage \
  -H "Authorization: Bearer YOUR_API_KEY"

# Plan limits (max workspaces, memory, monthly credits)
curl https://api.rigbox.dev/api/v1/users/me/limits \
  -H "Authorization: Bearer YOUR_API_KEY"
```
