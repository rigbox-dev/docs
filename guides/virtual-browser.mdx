...
If your workspace doesn't use the `full` image, install Virtual Browser first. This installs Playwright along with Chromium and Firefox browsers. The install runs asynchronously and prints a job id you can poll.
...
`rig workspace spawn` creates the workspace from the `full` template, boots it, and waits until it's ready:

```bash
rig workspace spawn --name browser-demo --template full
```
