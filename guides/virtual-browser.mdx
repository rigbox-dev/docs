...
Xvfb (virtual display)
  → Chromium or Firefox (renders pages)
    → x11vnc (streams display via VNC)
      → noVNC (browser-accessible VNC client)
        → Accessible at browser-{workspace-id}.rigbox.dev
```
...
See [Install Virtual Browser](/api-reference/tools/virtual-browser/install) for the full response schema.
...
See [Installation Status](/api-reference/tools/virtual-browser/install-status) for the response schema.
...
See [Launch Virtual Browser](/api-reference/tools/virtual-browser/launch) for the response schema.
...
See [Stop Virtual Browser](/api-reference/tools/virtual-browser/stop) for details.
