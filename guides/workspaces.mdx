...
A **workspace** is an isolated micro-VM running full Debian 12 Linux with systemd, SSH access, and its own private IP address. Each workspace has its own kernel and filesystem, with configured vCPU, RAM, and disk allocations. Physical host infrastructure is shared; a vCPU allocation does not promise a dedicated physical core.
...
Use the [Rigbox CLI](/cli-reference/cli) to create a workspace. Replace `<WORKSPACE_NAME>` with a name of your choice (for example, `my-project`):

```bash
rig workspace new --name <WORKSPACE_NAME> --template base --ram 512 --vcpu 1 --disk 3072
```

The resource and template flags are optional; `--name` is required. Omit the resource flags to use the selected template’s defaults. If requested resources are below the template or catalog floor, Rigbox raises them before creating the workspace. `rig workspace new` prints the new workspace's ID, status, and connection info on stdout.
...
```bash
rig workspace resize --workspace $WORKSPACE_ID --ram 1024 --vcpu 2 --disk 5120
```
...
Set workspace environment variables. The API saves them and updates guest configuration files for a running workspace; existing processes must reload their configuration or restart to use new values.

```bash
rig workspace env set --workspace $WORKSPACE_ID \
...
Read the latest CPU, RAM, and disk metrics. Repeat the command when you need a new sample.

```bash
rig workspace metrics --workspace $WORKSPACE_ID
```

For machine-readable output, add `--output json` and pipe to `jq`:

```bash
rig workspace metrics --workspace $WORKSPACE_ID --output json | jq
```
...
Read workspace logs or follow the log stream:
