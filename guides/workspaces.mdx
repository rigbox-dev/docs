...
```bash
rig new <WORKSPACE_NAME> --image base --ram 512 --vcpu 1 --disk 3072
```

The flags are optional — leave them off to take the account defaults. `rig new` prints the new workspace's ID, status, and connection info on stdout.
...
```bash
rig spawn <WORKSPACE_NAME> --template dev
```

`rig spawn` creates the workspace, starts it, and waits until it's ready — all in one call. See [Images & Templates](/guides/images-and-templates) for the list of templates, or [Quick Deploy](/api-reference/templates/quick-deploy) for the API.
...
```bash
rig start <WORKSPACE_NAME_OR_ID>
```

`rig start` polls the workspace status internally and returns once the VM is up. For scripting, `rig status <NAME>` prints the current state. See [Start Workspace](/api-reference/workspaces/start) for the API form.
...
```bash
rig logs <WORKSPACE_NAME_OR_ID>          # recent logs
rig logs <WORKSPACE_NAME_OR_ID> --follow # live stream
```
...
End-to-end with a single command — `rig spawn` creates the workspace, boots it, and waits until it's ready:

```bash
rig spawn demo-workspace --image base --ram 512 --vcpu 1 --disk 3072
```

When it returns, the workspace is running and SSH-reachable. Get the connection string with:

```bash
rig ssh-info demo-workspace
```
