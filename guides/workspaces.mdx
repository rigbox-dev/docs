---
title: Workspaces
description: Create, manage, and monitor isolated Firecracker micro-VM workspaces.
---

A **workspace** is an isolated Firecracker micro-VM running full Debian 12 Linux with systemd, SSH access, and its own private IP address. Each workspace gets dedicated CPU, RAM, and disk — nothing is shared with other workspaces.
...
```
provisioned → running → stopped → deleted
                ↑          |
                └──────────┘
                 (restart)
```
...
```python Python
requests.post(
    f"https://api.rigbox.dev/api/workspaces/{workspace_id}/start",
    headers={"Authorization": f"Bearer {api_key}"},
)
```
</CodeGroup>
...
while True:
    resp = requests.get(
        f"https://api.rigbox.dev/api/workspaces/{workspace_id}",
        headers={"Authorization": f"Bearer {api_key}"},
    )
...
```python Python
requests.post(
    f"https://api.rigbox.dev/api/workspaces/{workspace_id}/stop",
    headers={"Authorization": f"Bearer {api_key}"},
)
```
</CodeGroup>
...
```python Python
requests.put(
    f"https://api.rigbox.dev/api/workspaces/{workspace_id}/resources",
    headers={"Authorization": f"Bearer {api_key}"},
    json={
        "ram_mb": 1024,
        "vcpu_count": 2,
...
See [Resize Workspace](/api-reference/workspaces/resize) for the full schema.
...
```python Python
requests.post(
    f"https://api.rigbox.dev/api/workspaces/{workspace_id}/env",
    headers={"Authorization": f"Bearer {api_key}"},
    json={
        "env_vars": {
            "DATABASE_URL": "postgres://localhost:5432/mydb",
...
See [Set Environment Variables](/api-reference/workspaces/set-env) for details.
...
response = requests.get(
    f"https://api.rigbox.dev/api/workspaces/{workspace_id}/metrics",
    headers={"Authorization": f"Bearer {api_key}"},
)
...
See [Get Metrics](/api-reference/workspaces/metrics) for the full response schema.
...
curl -N https://api.rigbox.dev/api/workspaces/$WORKSPACE_ID/logs/stream \
  -H "Authorization: Bearer $RIGBOX_API_KEY"
```

```python Python (recent logs)
response = requests.get(
    f"https://api.rigbox.dev/api/workspaces/{workspace_id}/logs",
    headers={"Authorization": f"Bearer {api_key}"},
)
...
See [Get Logs](/api-reference/workspaces/logs) and [Stream Logs](/api-reference/workspaces/logs-stream) for details.
