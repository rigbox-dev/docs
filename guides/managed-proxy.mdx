...
To also pin the upstream provider and model, send the full config through `rig api`:
...
```bash
rig api PUT /v1/workspaces/ws_abc123/ai-config --body - <<'JSON'
{"mode": "managed", "provider": "google", "model": "gemini-2.5-flash-lite"}
JSON
```
