---
title: Blue-green deploys & promotion
description: Stage a change, verify it, then swap production over atomically — a whole-workspace preview clone or an in-VM sibling.
---

Stage a change and verify it before switching production routing. These preview workflows are distinct from managed incremental app releases. Explicit `strategy: incremental` rejects `--bluegreen` and `--promote`; use [stage and activate](/deploy/stage-and-activate) for that strategy.

For an existing preview workflow, retain its reviewed image/legacy configuration. Rehearse promotion and state handling on a test workspace first. There are two preview modes:

## `--bluegreen preview` — clone the whole workspace

Best for production **service hosts**. `preview` copy-on-write clones the **entire workspace** into a hidden preview VM, overlays your new code onto it, and serves it at `<app>-pv-<id>.rigbox.dev`. Production is untouched until you promote.

```bash
# Clone prod -> hidden preview, overlay this build, print the preview URL
rig deploy --bluegreen preview

# Verify the <app>-pv-<id> URL, then cut production over (route-swap):
rig deploy --bluegreen preview --promote
```

Because the preview is a separate VM, every app runs on its **same** declared port (no `$PORT` juggling), the whole multi-app workspace is previewed as a unit, and promote re-homes production onto the preview VM with **no restart** — an in-place edge route swap. The promoted VM inherits the **workspace name and SSH routing**, so subsequent name-based access resolves to the promoted workspace.

<Note>
Promote **swaps the underlying VM.** That's seamless for HTTP, but an open SSH session can't follow it — it stays on the old VM, which is retired later according to platform retention. The promote warns you if anyone's actively in the old VM, and the old VM is never auto-retired while a session is live. Re-attach with `rig ssh <workspace>` after promote to land on the new VM.
</Note>

**Rollback.** The old production VM is parked as `<name>-rollback-<id>` and kept as an instant rollback target until it's retired. To revert a promote before then, call the rollback endpoint on the promoted workspace:

```bash
rig rollback --workspace <workspace-id>
```

## `--bluegreen <suffix>` — in-VM sibling

Best for a workspace people actively work **in**. `<suffix>` stages a sibling app `<base>-<suffix>` on its own subdomain **inside the same workspace VM**. The VM never changes — so SSH sessions, files, and other processes survive — but apps must bind `$PORT`, and promote does a brief brief app restart.

```bash
rig deploy --bluegreen v2              # stage <base>-v2 on its own subdomain
rig deploy --bluegreen v2 --promote    # swap production to the sibling
```

`--promote` atomically swaps the production subdomain to the sibling, keeping the old app as a rollback target until you delete it.

## Which to use

| Your workspace is… | Use | Why |
| --- | --- | --- |
| A **production service host** — nobody's SSH'd in doing live work | `--bluegreen preview` | route-swap, same port everywhere, whole-workspace; old VM retired cleanly |
| A **live dev environment** — people SSH in, run processes, edit files | `--bluegreen <suffix>` or [deploy in place](/guides/deploying#deploy-modes) | the VM never changes, so sessions / files / in-VM state survive |

<Tip>
Review persistent data separately from code and routing. A preview is not an automatic database merge, and switching back does not reverse application writes. Use [image builds](/deploy/reproducible-builds) when a frozen environment is required.
</Tip>

## See also

- [Deploying with `rig deploy`](/guides/deploying) — the full deploy flow and modes.
- [Releases & rollback](/guides/releases-and-rollback) — frozen-image releases and `rig rollback`.
