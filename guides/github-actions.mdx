...
This guide uses Action `v1` and Rigbox CLI **v0.12.64 or newer**. GitHub OIDC deployments must be enabled for your Rigbox account. For automatic deployments without a workflow, connect a repository through the workspace creation page or the workspace’s GitHub deployment settings. Automatic pull-request preview environments are not yet supported.
