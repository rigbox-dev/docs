...
Then SSH in and run the agent — `rig ssh-info <workspace>` will print the right `ssh` command, or pipe it into [OpenRouter spawn](https://github.com/OpenRouterTeam/spawn) with `spawn link "$(rig ssh-info my-ws --format spawn-ssh)"`.
