...
`rig workspace spawn` creates the workspace from the `full` template (Architecture Explorer pre-installed), boots it, and waits until it's ready:

```bash
rig workspace spawn --name arch-demo --template full
```
