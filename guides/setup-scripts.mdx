...
When a workspace starts, Rigbox executes all attached setup scripts inside the VM as the `developer` user. Scripts run sequentially in the order you specify, with the workspace’s network access and sudo privileges.
...
Write the script to a local file, then create it with the CLI. The CLI create command uses the API’s default `first_boot` run mode; use the [Setup Scripts API](/api-reference/setup-scripts/create) when you need to explicitly choose `every_start`. Once created, attach its returned ID to the intended workspaces.
...
<Warning>
Check which workspaces reference a script before deleting it. Removing the stored definition does not undo changes made by an earlier run.
</Warning>
...
Read workspace logs after startup; there is no `setup_status` field to query on the workspace response:

```bash
rig workspace logs --workspace WORKSPACE_ID
```
...
Use the returned definition ID when attaching it to a workspace. After startup, inspect `rig workspace logs --workspace WORKSPACE_ID`; for service specs, check the installed unit with the commands above. A running VM does not prove a setup script or application completed successfully.
