...
response = requests.post(
    "https://api.rigbox.dev/api/v1/setup-scripts",
...
const response = await fetch("https://api.rigbox.dev/api/v1/setup-scripts", {
  method: "POST",
  headers: {
...
<CodeGroup>
```bash cURL
curl -s https://api.rigbox.dev/api/v1/setup-scripts \
...
```bash
curl -X PUT https://api.rigbox.dev/api/v1/setup-scripts/ss_abc123 \
  -H "Authorization: Bearer $RIGBOX_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE https://api.rigbox.dev/api/v1/setup-scripts/ss_abc123 \
  -H "Authorization: Bearer $RIGBOX_TOKEN"
```
...
```bash
curl -s https://api.rigbox.dev/api/v1/workspaces/{workspace_id} \
