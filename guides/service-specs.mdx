...
```bash
# Check status
sudo systemctl status rigbox-svc-api-server

# View logs
sudo journalctl -u rigbox-svc-api-server -f

# Manually start/stop/restart
sudo systemctl start rigbox-svc-api-server
sudo systemctl stop rigbox-svc-api-server
sudo systemctl restart rigbox-svc-api-server
```

<Note>
Service specs create system units under `/etc/systemd/system` with `User=developer`. Use `sudo systemctl` to manage the unit; the service process runs as the developer user.
</Note>
