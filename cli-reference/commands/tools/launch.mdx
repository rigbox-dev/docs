...
```bash
rig tools launch --tool TOOL --workspace WORKSPACE_ID
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig tools launch --help
```

## Local syntax

```text
Usage: rigbox tools launch [OPTIONS] --tool <TOOL> --workspace <WORKSPACE>
```
