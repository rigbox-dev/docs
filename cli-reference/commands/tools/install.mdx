---
title: "rig tools install"
sidebarTitle: "install"
description: "Install a tool into a workspace (async)"
---
...
```bash
rig tools install --workspace WORKSPACE_ID --tool TOOL
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig tools install --help
```

## Local syntax

```text
Usage: rigbox tools install [OPTIONS] --workspace <WORKSPACE> --tool <TOOL>
```
...
| `-t, --tool` | Tool id (e.g. architecture-explorer, virtual-browser) | Required; Value: TOOL |
...
```text
Install a tool into a workspace (async)
...
Options:
  ...
  -t, --tool <TOOL>
          Tool id (e.g. architecture-explorer, virtual-browser)
