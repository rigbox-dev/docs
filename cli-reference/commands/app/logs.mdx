...
```text
Usage: rigbox app logs [OPTIONS] --app <APP>
```
...
| `--install` | Show the app&#x27;s install logs (the rigbox-install-&lt;svc&gt; unit) instead of runtime logs. Works for CLI agents and failed/stopped apps | Default: false |
...
Options:
  ...
  -f, --follow
      ...
      --install
          Show the app's install logs (the rigbox-install-<svc> unit) instead of runtime logs. Works for CLI agents and failed/stopped apps
