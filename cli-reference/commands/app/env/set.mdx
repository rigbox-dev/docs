---
title: "rig app env set"
sidebarTitle: "set"
description: "Set app environment variables (preserves other keys; rig.yaml overrides on deploy)"
---
...
```bash
rig app env set --app APP_ID LOG_LEVEL=info
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig app env set --help
```

## Local syntax

```text
Usage: rigbox app env set [OPTIONS] --app <APP> <KVS>...
```
...
| `kvs` | Environment assignments (KEY=value); restart the app to apply | Required; Value: KVS |
...
```text
Set app environment variables (preserves other keys; rig.yaml overrides on deploy)

Usage: rigbox app env set [OPTIONS] --app <APP> <KVS>...

Arguments:
  <KVS>...
          Environment assignments (KEY=value); restart the app to apply
