---
title: "rig app param set"
sidebarTitle: "set"
description: "Set one or more parameter values"
---
...
```bash
rig app param set --app APP_ID KVS
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig app param set --help
```

## Local syntax

```text
Usage: rigbox app param set [OPTIONS] --app <APP> <KEY=VALUE>...
```
...
| `kvs` | One or more `KEY=VALUE` pairs (positional, repeatable) | Required; Value: KEY=VALUE |
...
Arguments:
  <KEY=VALUE>...
          One or more `KEY=VALUE` pairs (positional, repeatable)
