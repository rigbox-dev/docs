---
title: "rig app expose-port"
sidebarTitle: "expose-port"
description: "Detect or expose a workspace port as an app"
---
...
```bash
rig app expose-port --workspace WORKSPACE_ID --port 3000
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig app expose-port --help
```

## Local syntax

```text
Usage: rigbox app expose-port [OPTIONS] --workspace <WORKSPACE> --port <PORT>
```
...
| `-p, --port` | Port to detect or expose | Required; Value: PORT |
| `-n, --name` | App name to use when wrapping the port | Value: NAME |
| `--wrap` | Create the app route instead of only detecting the process | Default: false |
...
```text
Detect or expose a workspace port as an app
...
Options:
  ...
  -n, --name <NAME>
          App name to use when wrapping the port

      --cli-input-yaml <FILE>
          Read inputs from a YAML file or '-' (stdin)

      --wrap
          Create the app route instead of only detecting the process
