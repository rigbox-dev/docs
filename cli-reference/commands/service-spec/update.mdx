---
title: "rig service-spec update"
sidebarTitle: "update"
description: "Update an existing service spec from a YAML file"
---
...
```bash
rig service-spec update --id ID --file ./config.json
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig service-spec update --help
```

## Local syntax

```text
Usage: rigbox service-spec update [OPTIONS] --id <ID> --file <FILE>
```
...
| --- | --- | --- |
| `--id` | Service spec id (from `rig service-spec ls`) | Required; Value: ID |
| `-f, --file` | Path to the YAML spec file | Required; Value: FILE |
...
```text
Update an existing service spec from a YAML file

Usage: rigbox service-spec update [OPTIONS] --id <ID> --file <FILE>

Options:
      --id <ID>
          Service spec id (from `rig service-spec ls`)
