---
title: "rig app-release ls"
sidebarTitle: "ls"
description: "List retained app releases (distinct from workspace image releases)"
---
...
```bash
rig app-release ls --workspace WORKSPACE_ID
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig app-release ls --help
```

## Local syntax

```text
Usage: rigbox app-release ls [OPTIONS] --workspace <WORKSPACE>
```
...
```text
List retained app releases (distinct from workspace image releases)
