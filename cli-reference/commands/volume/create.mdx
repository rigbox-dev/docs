...
```bash
rig volume create --workspace WORKSPACE_ID --name my-project --size-mb 10240 --mount-path MOUNT_PATH
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig volume create --help
```

## Local syntax

```text
Usage: rigbox volume create [OPTIONS] --workspace <WORKSPACE> --name <NAME> --size-mb <SIZE_MB> --mount-path <MOUNT_PATH>
```
...
| `-n, --name` | Volume name | Required; Value: NAME |
| `--size-mb` | Volume size in MiB | Required; Value: SIZE_MB |
| `--mount-path` | Absolute mount path inside the workspace | Required; Value: MOUNT_PATH |
...
Options:
  ...
  -n, --name <NAME>
      ...
      --mount-path <MOUNT_PATH>
          Absolute mount path inside the workspace
