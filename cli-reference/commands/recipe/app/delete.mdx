---
title: "rig recipe app delete"
sidebarTitle: "delete"
description: "Delete published recipe version(s)"
---
...
```bash
rig recipe app delete --ref DEPLOY_REF
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig recipe app delete --help
```

## Local syntax

```text
Usage: rigbox recipe app delete [OPTIONS] --ref <DEPLOY_REF>
```
...
| --- | --- | --- |
| `-r, --ref` | Recipe ref. With a version (@vendor/slug@version) deletes that one version; with --all-versions pass the version-less @vendor/slug | Required; Value: DEPLOY_REF |
| `--all-versions` | Delete every published version of the recipe. The ref must NOT carry a version when this is set | Default: false |
...
```text
Delete published recipe version(s)
...
Options:
  ...
  -r, --ref <DEPLOY_REF>
          Recipe ref. With a version (@vendor/slug@version) deletes that one version; with --all-versions pass the version-less @vendor/slug

      --all-versions
          Delete every published version of the recipe. The ref must NOT carry a version when this is set
