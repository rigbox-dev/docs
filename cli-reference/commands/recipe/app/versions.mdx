...
```bash
rig recipe app versions --ref RECIPE_REF
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig recipe app versions --help
```

## Local syntax

```text
Usage: rigbox recipe app versions [OPTIONS] --ref <RECIPE_REF>
```
...
| --- | --- | --- |
| `-r, --ref` | Version-less recipe ref, e.g. @vendor/slug or vendor/slug | Required; Value: RECIPE_REF |
...
Options:
  ...
  -r, --ref <RECIPE_REF>
          Version-less recipe ref, e.g. @vendor/slug or vendor/slug
