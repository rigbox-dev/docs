---
title: "rig workspace env set"
sidebarTitle: "set"
description: "Set one or more environment variables on a workspace"
---
...
```bash
rig workspace env set PAIRS --workspace WORKSPACE_ID
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig workspace env set --help
```

## Local syntax

```text
Usage: rigbox workspace env set [OPTIONS] --workspace <WORKSPACE> <PAIRS>...
```
...
| --- | --- | --- |
| `pairs` | KEY=VALUE pairs (one or more) | Required; Value: PAIRS |
...
```text
Set one or more environment variables on a workspace
...
```text
Usage: rigbox workspace env set [OPTIONS] <PAIRS>...
```
