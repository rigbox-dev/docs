---
title: "rig workspace retry"
sidebarTitle: "retry"
description: "Retry a workspace that failed during creation"
---
...
```bash
rig workspace retry --workspace WORKSPACE_ID
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig workspace retry --help
```

## Local syntax

```text
Usage: rigbox workspace retry [OPTIONS] --workspace <WORKSPACE>
```
...
```text
Retry a workspace that failed during creation
