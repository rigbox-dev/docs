...
```bash
rig snapshot rm --workspace WORKSPACE_ID --snapshot SNAPSHOT
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig snapshot rm --help
```

Aliases: [remove](/cli-reference/commands/snapshot/rm).

## Local syntax

```text
Usage: rigbox snapshot rm [OPTIONS] --workspace <WORKSPACE> --snapshot <SNAPSHOT>
```
