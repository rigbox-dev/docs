---
title: "rig snapshot restore"
sidebarTitle: "restore"
description: "Restore a snapshot onto its workspace"
---
...
```bash
rig snapshot restore --workspace WORKSPACE_ID --snapshot SNAPSHOT
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig snapshot restore --help
```

## Local syntax

```text
Usage: rigbox snapshot restore [OPTIONS] --workspace <WORKSPACE> --snapshot <SNAPSHOT>
```
...
| `-s, --snapshot` | Snapshot id (from `rig snapshot ls`) | Required; Value: SNAPSHOT |
...
```text
Restore a snapshot onto its workspace
...
Options:
  ...
  -s, --snapshot <SNAPSHOT>
          Snapshot id (from `rig snapshot ls`)
