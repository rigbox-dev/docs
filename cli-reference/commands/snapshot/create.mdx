...
```bash
rig snapshot create --workspace WORKSPACE_ID
```

Inspect the supported arguments before running this command. Help is offline and does not modify resources.

```bash
rig snapshot create --help
```

## Local syntax

```text
Usage: rigbox snapshot create [OPTIONS] --workspace <WORKSPACE>
```
...
| `-n, --name` | Snapshot name (default: auto-generated) | Value: NAME |
