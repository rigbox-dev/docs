...
```text
Usage: rigbox api-key rm [OPTIONS] --id <ID>
```
...
| --- | --- | --- |
| `--id` | API key id (from `rig api-key ls`) | Required; Value: ID |
...
Options:
      --id <ID>
          API key id (from `rig api-key ls`)
