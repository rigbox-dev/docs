---
title: "rig build logs"
sidebarTitle: "logs"
description: "View host build logs for a build (the builder's install output)"
---
...
```text
Usage: rigbox build logs [OPTIONS] <BUILD_ID>
```
...
| --- | --- | --- |
| `build_id` | Build id (bld-…) printed by `rig deploy` / `rig build image` | Required; Value: BUILD_ID |
...
```text
View host build logs for a build (the builder's install output)

Usage: rigbox build logs [OPTIONS] <BUILD_ID>

Arguments:
  <BUILD_ID>
          Build id (bld-…) printed by `rig deploy` / `rig build image`
