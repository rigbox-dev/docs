...
Rigbox separates platform operations from user code. Workspaces run in micro-VMs with separate kernels and filesystems. Network controls and authenticated bridges mediate access to platform capabilities; see [security boundaries](/concepts/security) for the limits of this model.
...
- **VM networking** - each VM uses a private interface, with platform routing and network controls governing traffic
- **systemd init** - full Linux userspace with service management
- **Scoped credentials** - managed upstream provider keys remain at the proxy; workspace configuration can contain user-supplied secrets and scoped credentials

Code running as root can read and modify that workspace’s files and credentials. Treat each workspace as a trust boundary and avoid placing mutually untrusted applications or credentials in the same workspace.
...
- Rigbox-managed upstream provider keys stay at the proxy. BYOK credentials you supply are available to your workspace code
